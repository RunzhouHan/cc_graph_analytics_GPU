...
int main(int args, char **argv)
{
    ...
    if(args != 4)
    {
        printf("Usage: ./bfs <bw_beg_filename> <bw_csr_filename> <source>\n");
        exit(-1);
    }

    char *bw_beg_filename = argv[1];
    char *bw_csr_filename = argv[2];
    index_t root = atoi(argv[3]);

    graph *g = new graph(bw_beg_filename, bw_csr_filename);
